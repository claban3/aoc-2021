...
fn main() {
    ...
    lines.for_each(|l| {
        ...
        match (lx == rx, ly == ry) {
            ...
            // diagonal /
            (false, false) if (ry - ly) == (rx - lx) => {
                let hor_dist = (rx - lx).abs() + 1;
                let (startx, starty) = if rx < lx { (rx, ry) } else { (lx, ly) };
                (0..hor_dist).for_each(|i| {
                    *pt_counts.entry((startx + i, starty + i)).or_insert(0) += 1;
                });
            },
            // diagonal \
            (false, false) if (ry - ly) == -1 * (rx - lx) => {
                let hor_dist = (rx - lx).abs() + 1;
                let (startx, starty) = if rx < lx { (rx, ry) } else { (lx, ly) };
                (0..hor_dist).for_each(|i| {
                    *pt_counts.entry((startx + i, starty - i)).or_insert(0) += 1;
